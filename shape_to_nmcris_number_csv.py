...
def make_directory(path, directory_name):
    # directory_name = '/csv/nmcris-numbers'
...
def extract_column(output_directory, output_filename, df, column):
    ...
    with open(os.path.join(os.getcwd() + output_directory + '/' + output_filename), 'w') as f:
...
def shapefile_to_csv(filename, path):
    ...
    df.to_csv(create_filename(path + output_directory + '/' + filename.split('_')[0] + '_data', '.csv'), index=False)
